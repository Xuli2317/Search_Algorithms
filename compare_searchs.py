...
window = pyglet.window.Window(width=1100, height=800, caption='Search Algorithms Comparison')
batch = pyglet.graphics.Batch()

def reset_searches():
    global numbers, linear_index, linear_found, binary_left, binary_right, binary_mid, binary_found
    # Generate a list with random numbers and include 13, then sort it for binary search
    numbers = random.sample(list(range(1, 13)) + list(range(14, 50)), 20) + [13]
...
def update_searches(dt):
    ...
    if not binary_found and binary_left <= binary_right:
        binary_mid = (binary_left + binary_right) // 2
        if numbers[binary_mid] == 13:
            binary_found = True
        elif numbers[binary_mid] < 13:
